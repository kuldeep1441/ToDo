...
const HomePage = () => {
  ...
  const totalTasks = useSelector((state) => state.allTasks);
  const chosenDayTasks = useSelector((state) => state.chosenDayTasks);
  ...
  const categorizeTasksByWeek = (tasks) => {
    ...
    while (currentWeekStart <= lastDate) {
      const currentWeekEnd = endOfWeek(currentWeekStart);
      const weekLabel = `${format(currentWeekStart, "d MMM")} - ${format(
        currentWeekEnd,
        "d MMM"
      )}`;
      const tasksForWeek = tasks.filter((task) =>
      ...
      taskWeeks.push({
        label: weekLabel,
        open: tasksForWeek.filter((task) => task.status !== "Completed").length,
        completed: tasksForWeek.filter((task) => task.status === "Completed")
          .length,
        tasks: tasksForWeek,
        startDate: currentWeekStart,
      });
  ...
  const handleTaskStatusUpdate = async (id, currentStatus) => {
    const newStatus =
      currentStatus === "In Progress" ? "Completed" : "In Progress";
    await dispatch(updateTaskStatus(id, newStatus));
    dispatch(fetchTasks()); // Fetch tasks again to update list
  };
    ...
    return Array.from({ length: 7 }, (_, index) => {
      ...
      return (
        <div
          ...
          onClick={() => {
            setChosenDate(day);
            filterTasksForChosenDate(totalTasks, day); // Use totalTasks directly
          }}
          className={`flex flex-col items-center p-2 cursor-pointer ${
            day.toDateString() === chosenDate.toDateString()
              ? "bg-blue-500 text-white rounded-lg"
