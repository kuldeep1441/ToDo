...
const TaskForm = ({ initialData = {}, setShowTaskForm }) => {
  ...
  const formatDate = (date) => date.toISOString().split("T")[0];
  const [date, setDate] = useState(
    initialData.date ? formatDate(new Date(initialData.date)) : formatDate(new Date())
  );
  ...
  const [endTime, setEndTime] = useState(initialData.endTime || formatTime(new Date()));
  ...
  return (
    <form
      className="p-4 flex flex-col gap-4 border-t border-gray-200 bg-white rounded-lg shadow-lg w-full"
      onSubmit={handleSubmit}
    >
      <div className="flex justify-between items-center">
         <h2 className="text-lg font-semibold">Add New Task</h2>
